...
pub fn test_initialize() {
    ...
    let result = send(&mut svm, &[instruction], &payer, &[&payer]);
    assert!(result.is_ok());
}
...
pub fn test_deposit() {
    ...
    let deposit_ix = create_depoist_ix(
        &mut svm, &payer, mint_x, mint_y, config, mint_lp, vault_x, vault_y,
    );
    let result = send(&mut svm, &[init_ix, deposit_ix], &payer, &[&payer]);
    assert!(result.is_ok());
}
...
pub fn test_withdraw() {
    ...
    let depoist_ix = create_depoist_ix(
        &mut svm, &payer, mint_x, mint_y, config, mint_lp, vault_x, vault_y,
    );
    ...
    let result = send(
        &mut svm,
        &[init_ix, depoist_ix, withdraw_ix],
        &payer,
        &[&payer],
    );
    assert!(result.is_ok());
...
pub fn test_swap() {
    ...
    let result = send(&mut svm, &[init_ix, depoist_ix, swap_ix], &payer, &[&payer]);
    assert!(result.is_ok());
}
